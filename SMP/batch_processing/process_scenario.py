...
def solve_scenario(scenario, planning_problem, configuration_dict, status_dict, automaton, config: ScenarioConfig, result_dict) -> SearchResult:
    ...
    try:
        ...
        best_first = {MotionPlannerType.ASTAR, MotionPlannerType.UCS, MotionPlannerType.GBFS, MotionPlannerType.STUDENT}
        if (config.motion_planner_type in best_first):
            list_of_list_of_states, list_of_motion_primitives, _, final_status_dict = BestFirstSearch.execute_search(motion_planner, status_dict)
        else:
            print("Sorry, currently our tool is only compatible with the BestFirstSearch algorithm."
        ...
        if list_of_list_of_states is None:
            ...
            to_serializable_dict(status_dict, configuration_dict['setting']['output_path_json'] + str(scenario.scenario_id))

            #print(status_dict)
            #print(final_status_dict)
        else:
            search_time_ms = get_search_time_in_ms(time1)
...
def to_serializable_dict(results_dict, output_path):
    new_dict = {}
    for k in results_dict.keys():
        ...
        for state in state_list:
            new_state = {}
            new_state['position'] = list(state.position)
            new_state['orientation'] = state.orientation
            new_state['velocity'] = state.velocity
            new_state['time_step'] = state.time_step
            new_state_list.append(new_state)
        new_status = str(status)
        
        new_entry = (new_state_list, new_status)
        new_dict[k] = new_entry
    json.dump(new_dict, open(output_path + '.json', 'w' ))

def save_solution(scenario: Scenario, planning_problem_set: PlanningProblemSet, planning_problem_id: int,
                  config: ScenarioConfig,
                  computation_time_in_sec: float, list_of_list_of_states: List[List[State]], output_path: str = './',                               output_path_scenario: str = './solved_scenarios',
    ...
    if validate_solution:
        if valid_solution(scenario, planning_problem_set, solution)[0]:
            ...
            #hf.save_solution_to_scenario(scenario, planning_problem_set, pps.trajectory, output_path=output_path_scenario)
...
def process_scenario(scenario_id, scenario_loader: ScenarioLoader, configuration_dict, def_automaton: ManeuverAutomaton,
    ...
    try:
        ...
        p = Process(target=solve_scenario, args=(scenario, planning_problem, configuration_dict, status_dict, automaton, scenario_config, result_dict))
        p.start()
        ...
        if isinstance(result_dict[scenario_id], SearchResult):
            ...
            if search_result.result == ResultType.TIMEOUT:
                ...
                is_valid_solution = save_solution(scenario=scenario, planning_problem_set=planning_problem_set,
                                                  ...
                                                  output_path_scenario=configuration_dict['setting']['output_path_scenario'],
                                                  overwrite=hf.str2bool(
                                                      configuration_dict['setting']['overwrite']),
...
def debug_scenario(scenario_id, scenario_loader: ScenarioLoader, configuration_dict, def_automaton: ManeuverAutomaton,
    ...
    try:
        ...
        # Solving scenario
        solve_scenario(scenario, planning_problem, configuration_dict, automaton, scenario_config, result_dict)
